...
async def processPipeline(content, title, MODEL_PATH):
    """
    ...
    description = (
        "Dramatic_narrator, realistic male gravelly voice in the 30s age with american accent. "
        "Normal pitch, warm timbre, story telling fast pacing, neutral tone delivery at med intensity."
    )
...
def convert(Args, content, title):
    ...
    modelPath = Args.Generator.Maya.ModelPath

    files = []
    for part, chunk in enumerate(chunks):
        file = title+f"_{part}"
        asyncio.run(processPipeline(chunk,file, modelPath))
        files.append(file+'.wav')

    merge_audio(files, title, format="wave")


if __name__ == "__main__":
    # Run async main

    content = "Hello! This is a test of the Maya-1-Voice text-to-speech system."

    asyncio.run(processPipeline(content, "test"))
